...
export class CampusAlert extends LitElement {
  ...
  constructor() {
    ...
    if (localStorage.getItem('alertStatus') === 'closed') {
      this.open = false;
      this.toggleDisplayStyle(false);
    } else {
      this.toggleDisplayStyle(true);
    }
  }

  toggleDisplayStyle(openState) {
    if (openState) {
  ...
  static get styles() {
    return css`
      ...
      :host([status="notice"]) {
        --general-background-color: #f99806;
        --text-background-color: #fca728;
      }

      :host([status="warning"]) {
        --general-background-color: #f80202;
        --text-background-color: #ef4f4f;
      }

      :host([status="alert"]) {
        --general-background-color: #03f713;
        --text-background-color: #3ce747;
      }
